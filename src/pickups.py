...
class Item:
    ...
    def __init__(self, name, value=20, symbol="?", kind="fruit", required=True):
        self.name = name
        self.value = value
        self.symbol = symbol
        self.kind = kind
...
placed_items = [
    ...
    (7, 9, Item("cucumber")),
    (22, 10, Item("meatball")),
    (19, 5, Item("trap", value=-10, symbol="T", kind="trap", required=False)),
    ...
    (33, 10, Item("exit", value=0, symbol="E", kind="exit", required=False)),
]
...
def spawn_fruit(grid):
    for y in range(1, grid.height - 1):
        for x in range(1, grid.width - 1):
            if grid.is_empty(x, y) and not grid.is_player_position(x, y):
                grid.set(x, y, Item("fertile fruit", symbol="f", required=False))
                return True
    return False
